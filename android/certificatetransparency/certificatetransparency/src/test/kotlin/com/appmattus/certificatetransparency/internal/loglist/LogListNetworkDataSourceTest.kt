...
internal class LogListNetworkDataSourceTest {
    ...
    private val logListService: LogListService = TestLogListService(retrofit.create(TestRetrofitService::class.java))
    ...
    private fun expectInterceptorHttpNotFound(url: String) {
        whenever(mockInterceptor.intercept(argThat { request().url().toString() == url })).then {
            ...
            Response.Builder()
                .body(ResponseBody.create(MediaType.get("application/octet-stream"), ByteArray(0)))
                .request(chain.request())
                .protocol(Protocol.HTTP_2)
                .code(404)
                .message("")
    ...
    private fun expectInterceptor(
        ...
        whenever(mockInterceptor.intercept(argThat { request().url().toString() == url })).then {
            ...
            Response.Builder()
                .body(ResponseBody.create(MediaType.get("application/octet-stream"), byteResponse))
                .request(chain.request())
                .protocol(Protocol.HTTP_2)
                .code(200)
                .message("")
