...
static int cbb_init(CBB* cbb, uint8_t* buf, size_t cap) {
  ...
  base = static_cast<cbb_buffer_st*>(malloc(sizeof(struct cbb_buffer_st)));
  if (base == NULL) {
    return 0;
  }
...
int CBB_init(CBB* cbb, size_t initial_capacity) {
  ...
  uint8_t* buf = reinterpret_cast<uint8_t*>(malloc(initial_capacity));
...
static int cbb_buffer_reserve(struct cbb_buffer_st* base,
  ...
  if (newlen > base->cap) {
    ...
    newbuf = reinterpret_cast<uint8_t*>(realloc(base->buf, newcap));
    if (newbuf == NULL) {
      goto err;
    }
...
int CBB_flush(CBB* cbb) {
  ...
  if (!CBB_flush(cbb->child) || child_start < cbb->child->offset ||
  ...
  if (cbb->child->pending_is_asn1) {
    ...
    if (len_len != 1) {
      ...
      safe_memmove(cbb->base->buf + child_start + extra_bytes,
...
static int cbb_add_length_prefixed(CBB* cbb,
  ...
  safe_memset(prefix_bytes, 0, len_len);
  safe_memset(out_contents, 0, sizeof(CBB));
...
int CBB_reserve(CBB* cbb, uint8_t** out_data, size_t len) {
  if (!CBB_flush(cbb) || !cbb_buffer_reserve(cbb->base, out_data, len)) {
    return 0;
  }
  return 1;
}
...
int CBB_add_u16le(CBB* cbb, uint16_t value) {
  return CBB_add_u16(cbb, __builtin_bswap16(value));
}
...
int CBB_add_u32le(CBB* cbb, uint32_t value) {
  return CBB_add_u32(cbb, __builtin_bswap32(value));
}
...
int CBB_add_u64le(CBB* cbb, uint64_t value) {
  return CBB_add_u64(cbb, __builtin_bswap64(value));
}
...
static int parse_dotted_decimal(CBS* cbs, uint64_t* out) {
  *out = 0;
  ...
  for (;;) {
    ...
    // string.
    uint8_t u;
    if (!CBS_get_u8(cbs, &u) || (u == '.' && CBS_len(cbs) > 0)) {
      break;
    }
    if (u < '0' || u > '9' ||
        ...
        (seen_digit && *out == 0) ||
        // Check for overflow.
        *out > UINT64_MAX / 10 || *out * 10 > UINT64_MAX - (u - '0')) {
      return 0;
    }
    *out = *out * 10 + (u - '0');
    seen_digit = 1;
...
static int compare_set_of_element(const void* a_ptr, const void* b_ptr) {
  ...
  const CBS* b = reinterpret_cast<const CBS*>(b_ptr);
  size_t a_len = CBS_len(a), b_len = CBS_len(b);
  size_t min_len = a_len < b_len ? a_len : b_len;
  int ret = safe_memcmp(CBS_data(a), CBS_data(b), min_len);
  if (ret != 0) {
    return ret;
  }
  if (a_len == b_len) {
...
int CBB_flush_asn1_set_of(CBB* cbb) {
  ...
  uint8_t* buf =
      reinterpret_cast<uint8_t*>(safe_memdup(CBB_data(cbb), buf_len));
  CBS* children = reinterpret_cast<CBS*>(malloc(num_children * sizeof(CBS)));
